...
def run_onnx_inference(onnx_path, image_tensor):
    """Run inference using ONNX model and return raw outputs."""
    ...
    # Convert tensor to numpy (single image from list)
    if isinstance(image_tensor, list):
        image_np = image_tensor[0].unsqueeze(0).detach().cpu().numpy()  # Add batch dimension
    else:
        image_np = image_tensor.unsqueeze(0).detach().cpu().numpy()

    print(f"ONNX input shape: {image_np.shape}")

    # Run inference
    ort_inputs = {input_name: image_np}
    ort_outputs = ort_session.run(None, ort_inputs)

    # Convert outputs back to tensors for compatibility
    raw_outputs = {
...
def load_and_preprocess_image(image_path, target_height=None, target_width=None):
    ...
    try:
        ...
        # Resize if target dimensions provided (for ONNX model)
...
def run_inference_comparison(image_path, onnx_path=None, height=None, width=None, confidence_threshold=0.5, tolerance=1e-4):
    """Run inference comparison between original FCOS and ONNX model + PostProcessor."""
    ...
    print(f"=== FCOS ONNX vs Original Comparison ===")
    print(f"Test image: {image_path}")
    if onnx_path:
        print(f"ONNX model: {onnx_path}")
        print(f"Input dimensions: {height}x{width}" if height and width else "Using original image size")
    print(f"Confidence threshold: {confidence_threshold}")
    print(f"Comparison tolerance: {tolerance}")

    # Load and preprocess image
    if onnx_path and height and width:
        # For ONNX model, use specific dimensions
        test_images, original_size = load_and_preprocess_image(image_path, height, width)
    else:
        # For PyTorch model, use original size
    ...
    print("\n=== Running Custom Model (ONNX or FCOSBackbone) + PostProcessor ===")

    if onnx_path:
        # Use ONNX model
        print("Using ONNX model for inference...")
        raw_outputs = run_onnx_inference(onnx_path, test_images)
    else:
        # Use FCOSBackbone (fallback)
        print("Using FCOSBackbone for inference...")
    ...
    # Create post-processor with original model parameters
    ...
    model_name = "ONNX" if onnx_path else "FCOSBackbone"
    print(f"✓ Custom {model_name} + PostProcessor inference completed!")
    ...
    # 5. Summary
    print(f"\n=== Final Summary ===")
    model_type = "ONNX model" if onnx_path else "FCOSBackbone"

    if results_match:
        print(f"🎉 PERFECT: {model_type} + postprocessing implementation matches original FCOS!")
        print("Your implementation is working correctly.")
    else:
        print(f"❌ MISMATCH: {model_type} + postprocessing differs from original FCOS.")
        if onnx_path:
            print("This could indicate numerical precision issues in the ONNX model.")
        print("Check the implementation for potential issues.")
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Verify FCOS postprocessing with ONNX model or FCOSBackbone')
    parser.add_argument('--image-path', type=str, required=True,
                        help='Path to test image')
    parser.add_argument('--onnx-path', type=str, default=None,
                        help='Path to ONNX model file (if not provided, uses FCOSBackbone)')
    parser.add_argument('--height', type=int, default=None,
                        help='Input height for ONNX model (required if using ONNX)')
    parser.add_argument('--width', type=int, default=None,
                        help='Input width for ONNX model (required if using ONNX)')
    ...
    # Validate arguments
    if args.onnx_path and (args.height is None or args.width is None):
        print("Error: --height and --width are required when using ONNX model")
        exit(1)
    ...
    if args.onnx_path:
        print("If results match, your ONNX model and postprocessing are working correctly!")
    else:
        print("If results match, your FCOSBackbone postprocessing implementation is correct!")
