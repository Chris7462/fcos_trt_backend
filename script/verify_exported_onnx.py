...
def compare_outputs(pytorch_outputs, onnx_outputs, tolerance=1e-4):
    ...
    for key in pytorch_outputs.keys():
        ...
        if pytorch_tensor.shape != onnx_tensor.shape:
            print(f"✗ {key}: Shape mismatch! PyTorch: {pytorch_tensor.shape}, ONNX: {onnx_tensor.shape}")
            all_close = False
            continue
        ...
        if is_close:
            ...
            for i, idx in enumerate(max_diff_indices[:5]):
                idx = idx.item()
                print(f"    [{idx}]: PyTorch={flat_pytorch[idx]:.8f}, ONNX={flat_onnx[idx]:.8f}, diff={flat_diff[idx]:.8f}")
