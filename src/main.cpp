...
int main(int argc, char* argv[])
{
  ...
  try {
    ...
    //postprocessor.print_detection_results(detection_results, 10);

    // Print first 10 values for verification (matching Python script)
    std::cout << "\n=== First 10 Values for Verification ===" << std::endl;

    std::cout << "Boxes (first 10):" << std::endl;
    int box_count = std::min(10, static_cast<int>(detection_results.boxes.size()));
    ...
    std::cout << "Scores (first 10):" << std::endl;
    int score_count = std::min(10, static_cast<int>(detection_results.scores.size()));
    ...
    std::cout << "Labels (first 10):" << std::endl;
    int label_count = std::min(10, static_cast<int>(detection_results.labels.size()));
